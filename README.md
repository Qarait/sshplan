...
`sshplan` is a tiny OpenSSH access compiler. It takes a YAML policy, evaluates a request, and writes native OpenSSH artifacts you can inspect before anyone signs a certificate.
...
| Tool | Best at | sshplan difference |
| --- | --- | --- |
| Static SSH keys | Simple login | sshplan plans short-lived certificate issuance |
| Vault SSH CA | Secrets platform CA | sshplan is a local compiler, not a secrets platform |
| Smallstep | Certificate infrastructure | sshplan focuses on OpenSSH access artifacts |
| Teleport | Full access platform | sshplan avoids proxy, agent, and daemon scope |

## Lineage

From the creator of Ephemera, Gate0, and Gate1. Ephemera explored self-hosted SSH certificate access. Gate0 and Gate1 explored bounded authorization kernels. sshplan compresses those lessons into a small OpenSSH access compiler.
